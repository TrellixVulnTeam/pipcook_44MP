...
contract ERC20 is Context, IERC20 {
     ...
     * 将发送地址和接收地址的金额数量相加减，就是一个买入函数,成交金额 = amount
     */
     ...
     * `amount`.
     * sender(发送人地址) recipient(接收者地址) amount(代币数量)
     * 先对地址的代币金额交换
     * 后存入上次存入代币数量 - 当次代币数量,并写入事件
     */
     ...
     * - `recipient` cannot be the zero address.
     * - `sender` must have a balance of at least `amount`.
     * 代币数量的交换(sender发送者减少代币，recipient接受者增加代币),并写入事件日志
     */
     ...
     * - `spender` cannot be the zero address.
     * 存储 owner地址的spender(花费地址)的amount(代币数量),并写日志
     */
     ...
     * 将account 地址的代币数量减去，存储account地址的sender(地址发送人)的代币余下数量(之前存储的数量 - amount)
     */
 ...
 * 这个contract 均是移除地址的代币数量的功能 即烧钱,人为制造通缩，代币才能升值
 */
